...
my @filetypesToProcess = qw( pl pm );
# my @filetypesToProcess = qw( c h go bash sh rs pl pm py cs ts );
my $inputDirectory = '../../corpus/code/';
my $outPath = './out/';
...
our $opt_i;
getopts('f:i:');
@filetypesToProcess = split / /, $opt_f if $opt_f;
$inputDirectory = $opt_i if $opt_i;
...
my $inputSubfolder = "";
$inputSubfolder = $opt_i if $opt_i;
# print STDERR $inputSubfolder;
$inputSubfolder =~ s/.*\/(.*?)\/$/$1/;
$inputSubfolder = "_" . $inputSubfolder;
...
for (my $i = 0; $i <= $#filetypesToProcess; $i++) { 
	...
	my $filetype = '*.' . $filetypesToProcess[$i];
#	$filetype = '*.' . $opt_f if $opt_f;
	...
	foreach (@files) {
		$numProcessed++;
		last if ($debugMode && $numProcessed > 10);
		my $p = 0;
		$fname = $_;
		...
		while(<$in>) {
			...
			print STDERR "$fname ($numProcessed / $numFiles): $p\n" if ($p % 100 == 0);
	...
	my @mgramsLetters = sort by_mgramsLetters keys %mgramsLetters if $doLetters;
	my @bgrams = sort by_bgrams keys %bgrams if $doDis;
	my @tgrams = sort by_tgrams keys %tgrams if $doTris;
	my $fileExt = uc $filetypesToProcess[$i];
	$fileExt .= $inputSubfolder if $opt_i;
	if ($doLetters) {
		open(my $out, '>', $outPath . "allLetters$fileExt.txt") or die $!;
		select $out;
	...
	open(my $out, '>', $outPath . "allChars$fileExt.txt") or die $!;
	select $out;
	...
	if ($doDis) {
		open($out, '>', $outPath . "allDigraphs$fileExt.txt") or die $!;
		select $out;
	...
	if ($doTris) {
		open($out, '>', $outPath . "allTrigraphs$fileExt.txt") or die $!;
		select $out;
...
sub filterComments {
	...
	if ($fname =~ /\.c$/ || $fname =~ /\.h$/ || $fname =~ /\.cs$/ || $fname =~ /\.go$/ || $fname =~ /\.ts$/) {
		s/^\s*\*\s.*//;
		s/\/\*.*/\/*/;
		s/.*\/\*/*\//;
	}
...
sub filterOneLineComments {
	if ($fname =~ /\.rs$/ || $fname =~ /\.c$/ || $fname =~ /\.h$/ || $fname =~ /\.cs$/ || $fname =~ /\.go$/ || $fname =~ /\.ts$/) {
		$commentChars = '\/\/';
