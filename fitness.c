...
int calcFitness(Keyboard *k, int generateDetailedOutput)
{
	...
	char layoutBak[ksize * 3 + 1];
	layoutBak[0] = '\0';
	strcat(layoutBak, k->layout);
	strcat(layoutBak, k->shiftedLayout);
	strcat(layoutBak, k->symbolLayer);
	copyKeyboard(k, &nilKeyboard);
	setLayout(k, layoutBak);
	k->fitness = 0;
	...
	if (generateDetailedOutput) { 
		...
		for (i = 0; i < diLen; ++i) {
			...
			if (hand[loc0] == hand[loc1] && finger[loc0] == finger[loc1] && loc0 != loc1
				&& (!useRingForTopPinky || calcSameFinger(loc0, loc1, loc0 / ksize, loc1 / ksize) != 0)
			) k->sameFingerMax += multiplier;
		}
...
void calcAesthetics(Keyboard *k) {
	...
	cluster(k, (char[]){'*', '/', '%'}, 3, aestheticsMajor);
	// clusterPairs(k, (char[]){'+', '-', '*', '/'}, 4, aestheticsMajor);
	alignPairs(k, (char[]){'(', ')', '{', '}', '[', ']'}, 6, aestheticsMajor);
	// alignPairs(k, (char[]){'(', ')', '{', '}', '[', ']', '<', '>'}, 8, aestheticsMajor * 1);
...
void applyAdHocPenalties(Keyboard *k, double cost)
{
	...
	int loc4 = locWithShifted(k, '+');
	int loc5 = locWithShifted(k, '@');
	int loc6 = locWithShifted(k, '$');
	int loc7 = locWithShifted(k, '=');
	int loc8 = locWithShifted(k, '`');


	// if (row[loc8] != 0) penalty += 1;
	// if (loc3 / ksize == 0) penalty += 2;
	if (hand[loc0] == hand[loc1] && finger[loc0] == finger[loc1] && row[loc0] != row[loc1]) penalty += 1;
	// if (finger[loc7] != RING) penalty += 1;
	// if (hand[loc6] != RIGHT || row[loc6] != 1 || !isCenter[loc6]) penalty += 1;
	// if (isCenter[loc4]) penalty += 1;
	// if (loc6 / ksize == 0) penalty += 2;
	// if (loc5 / ksize != 0) penalty += 10;
...
inline int calcSameFinger(int loc0, int loc1, int layer0, int layer1)
{
	...
	if (useRingForTopPinky && ((finger[loc0] == RING && column[loc0] == 0) || (finger[loc1] == RING && column[loc1] == 0))) {
		if (((row[loc0] == 0 && column[loc0] == 1) || (row[loc1] == 0 && column[loc1] == 1)) && finger[loc0] != MIDDLE && finger[loc1] != MIDDLE) return 0;
	}
	...
	if ((isCenter[loc0] || isCenter[loc1]) && !(isCenter[loc0] && isCenter[loc1])) mult += 1;
...
inline int calcRowChange(int loc0, int loc1, int layer0, int layer1)
{
	...
	if ((fin0 == RING && column[loc0] == 0) || (fin0 == RING && column[loc0] == 0)) { // this is hacky and may not work as intended if the below tables change
		base += 1;
		if (fin0 == MIDDLE || fin0 == INDEX || fin1 == MIDDLE || fin1 == INDEX) base += 2;
	}
...
{
	if (layer0 != 0 && layer1 != 0 && hand[loc0] != hand[loc1] && biLayerSplit != 0) return biLayerSplit; 
	else return 0;
}
		...
		switch (uf) {
			...
			case RING:
				if (ufc == 1) {
					...
					else if (lf == INDEX && center) base = 4;		
				}
				break;
			case MIDDLE:
				if (lf == PINKY) base = 1;
				else if (lf == RING) base = 1;
				else if (lf == INDEX && !center) base = 0.5;
				else if (lf == INDEX && center) base = 2;
				break;
			case INDEX:
				if (!center) {
		...
		switch (uf) {
			...
			case INDEX:
				if (!center) {
					...
					else if (lf == MIDDLE) base = 3;
				}
				break;
		}
	}	

	return (int) (halfScissor * base);
}
...
{
	// note: cases where the keys are not on the same row are handled under the scissor logic
	if (row[loc0] != row[loc1] || finger[loc0] == finger[loc1]) return 0;
	if (!(isCenter[loc0] || isCenter[loc1])) return 0;

	int base = 0;
	int ringForTopPinky = ((finger[loc0] == RING && column[loc0] == 0) || (finger[loc1] == RING && column[loc1] == 0));

	if (row[loc0] == 0) {
		if ((finger[loc0] == PINKY || finger[loc1] == PINKY) && !ringForTopPinky) base = 0; 
		if ((finger[loc0] == PINKY || finger[loc1] == PINKY) && ringForTopPinky) base = 4; 
		if ((finger[loc0] == RING || finger[loc1] == RING)) base = 0; 
		if ((finger[loc0] == MIDDLE || finger[loc1] == MIDDLE)) base = 1; 
	}

	else if (row[loc0] == 1) {
		if ((finger[loc0] == PINKY || finger[loc1] == PINKY)) base = 0; 
		if ((finger[loc0] == RING || finger[loc1] == RING)) base = 1; 
		if ((finger[loc0] == MIDDLE || finger[loc1] == MIDDLE)) base = 2; 
	}

	else {
		if ((finger[loc0] == PINKY || finger[loc1] == PINKY)) base = 1; // 2, 3, 3
		if ((finger[loc0] == RING || finger[loc1] == RING)) base = 2; 
		if ((finger[loc0] == MIDDLE || finger[loc1] == MIDDLE)) base = 2; 	
	}
